...
```
小数组（kernel 启动开销主导）:
N=256:    0.168 ms  (NumPy: 0.012 ms, 0.07x - 比 NumPy 慢)
N=512:    0.254 ms  (NumPy: 0.016 ms, 0.06x - 比 NumPy 慢)
N=1024:   0.281 ms  (NumPy: 0.022 ms, 0.08x - 比 NumPy 慢)

大数组（并行计算优势显现）:
N=8192:   0.297 ms  (NumPy: 0.161 ms, 0.54x - 仍未超越)
N=16384:  0.319 ms  (NumPy: 0.333 ms, 1.05x - 开始超越)
N=32768:  0.536 ms  (NumPy: 0.725 ms, 1.35x faster)
N=65536:  0.974 ms  (NumPy: 2.567 ms, 2.63x faster) ⚡

⚠️ 注意：当前 CUDA 实现未经优化，性能远低于理论值
```
...
| 大小 | NumPy | Python CT | C++ | CUDA (Tesla T4) |
|------|-------|----------|-----|-----------------|
| 64   | 0.039ms | - | **0.0014ms (28x)** | - |
| 128  | 0.0066ms | 0.36ms | **0.0024ms (2.7x)** | - |
| 256  | **0.012ms** | 0.72ms | 0.0052ms (2.3x) | 0.168ms (0.07x) ⚠️ |
| 512  | **0.016ms** | 1.58ms | 0.011ms (1.5x) | 0.254ms (0.06x) ⚠️ |
| 1024 | **0.022ms** | 3.46ms | 0.024ms | 0.281ms (0.08x) ⚠️ |
| 4096 | **0.070ms** | - | 0.100ms | 0.333ms (0.21x) ⚠️ |
| 8192 | **0.161ms** | - | - | 0.297ms (0.54x) ⚠️ |
| 16384| **0.333ms** | - | - | 0.319ms (1.05x) |
| 32768| 0.725ms | - | - | **0.536ms (1.35x)** |
| 65536| 2.567ms | - | - | **0.974ms (2.63x)** ⚡ |

**结论**:

- **N ≤ 1024**: C++ 最快（小数组优化好）
- **1024 < N < 16384**: NumPy 最快（高度优化的库）
- **N ≥ 32768 (有GPU)**: CUDA 开始显现优势
- ⚠️ **CUDA 当前实现需要优化**：小数组性能差，大数组未达理论值
